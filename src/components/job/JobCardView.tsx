...
interface JobCardViewProps {
  ...
  userNames: {
    [userId: string]: {
      userFirstName?: string | 'Beshir';
      userLastName?: string | 'Dekebo';
      userEmail?: string;
    };
  };
    ...
    <Card variant="outline" borderWidth="1px" borderRadius="md" p={4}>
      ...
      <CardFooter>
        <ButtonGroup spacing="3">
          {hasAccess('application', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && status !== 'Rejected' && (
            <Button isDisabled={status === 'Hired'} colorScheme="primary" onClick={() => onHire(application)}>
              {status === 'Hired' ? 'Hired' : 'Hire'}
            </Button>
          )}
          {hasAccess('application', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && status !== 'Hired' && (
            <Button
